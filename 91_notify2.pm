...
notify2_Initialize($)
{
  ...
  $hash->{DefFn} = "notify2_Define";
  $hash->{NotifyFn} = "notify2_Notify";
  $hash->{AttrFn}   = "notify2_Attr";
...
{
  my ($hash, $def) = @_;
  my ($name, $type, $devspec, $command) = split("[ \t]+", $def, 4);

  if(!$command) {
    if($hash->{OLDDEF}) { # Called from modify, where command is optional
      (undef, $command) = split("[ \t]+", $hash->{OLDDEF}, 2);
      $hash->{DEF} = "$devspec $command";
    } else {
      return "Usage: define <name> notify2 <devspec> <command>";
    }
  }
  ...
  # Update a reading named "TriggerDevices" which lists all devices defined by users devspec
  readingsSingleUpdate($hash, "TriggerDevices", join(",", @rets), 0);
...
notify2_Notify($$)
{
  ...
  return if(!$events); # Some previous notify2 deleted the array.
  my $max = int(@{$events});
  my $t = $dev->{TYPE};
  ...
  my $found = 0;
  foreach my $sdev (devspec2array(ReadingsVal($ln, "TriggerDevices", ""))) {
    if ($sdev eq $n) {
      $found = 1;
    }
  }
  ...
  for (my $i = 0; $i < $max; $i++) {
    ...
    if($found) {
      Log3 $ln, 5, "notify2: Triggering $ln";
      my (undef, $exec) = split("[ \t]+", $hash->{DEF}, 2);
      ...
      Log3 $ln, 4, "notify2: $ln exec $exec";
      my $r = AnalyzeCommandChain(undef, $exec);
      Log3 $ln, 3, "notify2: $ln return value: $r" if($r);
      $ret .= " $r" if($r);
      $hash->{STATE} =
...
<ul>
  ...
  <a name="notify2define"></a>
  <b>Define</b>
  <ul>
    <code>define &lt;name&gt; notify2 &lt;pattern&gt; &lt;command&gt;</code>
    <br><br>
    ...
    Examples:
    <ul>
      <code>define b3lampV1 notify2 btn3 set lamp $EVENT</code><br>
      <code>define b3lampV2 notify2 btn3 { fhem "set lamp $EVENT" }</code><br>
      <code>define b3lampV3 notify2 btn3 "/usr/local/bin/setlamp "$EVENT""</code><br>
      <code>define b3lampV3 notify2 btn3 set lamp1 $EVENT;;set lamp2 $EVENT</code><br>
      <code>define wzMessLg notify2 wz:measured.* "/usr/local/bin/logfht $NAME "$EVENT""</code><br>
      <code>define LogUndef notify2 global:UNDEFINED.* "send-me-mail.sh "$EVENT""</code><br>
    </ul>
    <br>
    ...
    <ul>
      ...
      <li>notify2 can be used to store macros for manual execution. Use the <a
          href="#trigger">trigger</a> command to execute the macro.
          E.g.<br>
          <code>fhem> define MyMacro notify2 MyMacro { Log 1, "Hello"}</code><br>
  ...
  <ul>
    ...
    <li>forwardReturnValue<br>
        ...
        triggers this notify2 will also return this value. This can cause e.g
    ...
    <li>showTriggerTime<br/>
        ...
        notify2 instance. Default is 1 (on).
        </li>
...
<ul>
  ...
  <b>Define</b>
  <ul>
    <code>define &lt;name&gt; notify2 &lt;Suchmuster&gt; &lt;Anweisung&gt;</code>
    <br><br>
    ...
    <ul>
      <code>define b3lampV1 notify2 btn3 set lamp $EVENT</code><br>
      <code>define b3lampV2 notify2 btn3 { fhem "set lamp $EVENT" }</code><br>
      <code>define b3lampV3 notify2 btn3 "/usr/local/bin/setlamp
      "$EVENT""</code><br>

      <code>define b3lampV3 notify2 btn3 set lamp1 $EVENT;;set lamp2
      $EVENT</code><br>

      <code>define wzMessLg notify2 wz:measured.* "/usr/local/bin/logfht $NAME
      "$EVENT""</code><br>

      <code>define LogUndef notify2 global:UNDEFINED.* "send-me-mail.sh
      "$EVENT""</code><br>
    ...
    <ul>
      ...
      <li>In der Anweisung von notify2 kann das ausl&ouml;sende Ereignis (Event)
        genutzt werden:
      ...
      <li>notify2 kann dazu benutzt werden, um Makros f&uuml;r eine manuelle
        Ausf&uuml;hrung zu speichern. Mit einem <a
        href="#trigger">trigger</a> Kommando k&ouml;nnen solche Makros dann
        ausgef&uuml;hrt werden.  Z.B.<br> <code>fhem> define MyMacro notify2
  ...
  <ul>
    ...
    <li>addStateEvent<br>
      ...
      Achtung:
      <ul>
        <li>dieses Attribut muss beim Empf&auml;nger (notify2, FileLog, etc)
        gesetzt werden.</li>
